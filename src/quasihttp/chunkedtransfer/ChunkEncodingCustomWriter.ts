...
import { ICustomWriter, customWriterSymbol } from "../../common/types";
 ...
 */
export function createChunkEncodingCustomWriter(wrappedWriter: any,
        maxChunkSize = 0) {
    if (!wrappedWriter) {
    ...
    const writeAsync = async function(chunk: Buffer) {
        if (!Buffer.isBuffer(chunk)) {
            throw IOUtils.createNonBufferChunkError(chunk)
        }
        const offset = 0
        const length = chunk.length
        let bytesWritten = 0;
        while (bytesWritten < length) {
            bytesWritten += await writeNextSubsequentChunk(chunk,
    ...
    const endWritesAsync = async function() {
    ...
    const instance: ICustomWriter & {
            endWrites: () => Promise<void> 
        } = {
